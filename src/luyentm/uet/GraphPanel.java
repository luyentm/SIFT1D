...
public class GraphPanel extends JPanel {
	...
	public ArrayList<Integer> extremaMax = new ArrayList<Integer>();
	public ArrayList<Integer> mKeypoints = new ArrayList<Integer>();
	...
	public GraphPanel(ArrayList<ExtremaPoint> extrema,
			ArrayList<KeyPoint> keypoints) {
		// this.scores = scores;

		for (int i = 0; i < keypoints.size(); i++) {
			mKeypoints.add(keypoints.get(i).getPosition());

		}
		for (int i = 0; i < extrema.size(); i++) {
			extremaMax.add(extrema.get(i).getPosition());
		}
		LoadData1D gen = new LoadData1D(CF.inputFileName);
		ArrayList<Double> inputSignal = gen.getData();
		// sinh ra mot luong du lieu nua ve vi tri cua cac keypoints
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < graphPoints.size(); i++) {
			...
			int x = graphPoints.get(i).x - ovalW / 2;
			int y = graphPoints.get(i).y - ovalW / 2;
	...
	public static void add(double plus) {
		...
		System.out.println("Change Scale: " + add);
	}
	...
	public void Show() {
		...
		// int maxScore = 100;
		// day la cho de ma tao ra du lieu
		...
		JFrame frame = new JFrame("Luyentm - Tool hien thi du lieu SIGNAL");
